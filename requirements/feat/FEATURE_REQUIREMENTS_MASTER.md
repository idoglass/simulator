...
| FR-GR-058 | GR-058 | Implement features under MVC architecture with `py-gui`/`tk-mvc` for GUI and Textual for TUI, with explicit adapters boundary grouping. | Feature design maps to MVC layers, follows framework standards, documents `adapters/ui` and `adapters/transport` as boundary-role grouping, and includes compatibility validation for submodule updates. | P0 |
